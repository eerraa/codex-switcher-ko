...
const output = path.resolve('.windows-smoke.local');
fs.mkdirSync(output, { recursive: true });
const profile = fs.mkdtempSync(path.join(output, 'profile-'));
const fixture = path.resolve('scripts/fixtures/windows-ipc.mjs');
const korean = process.argv.includes('--korean');
const server = await createServer({
  logLevel: 'error', optimizeDeps: { entries: ['scripts/fixtures/windows-compat.html'] },
  server: { host: '127.0.0.1', port: 0, strictPort: false, open: false, watch: { ignored: ['**/*.local/**'] } },
  plugins: [{ name: 'windows-compat-fixture', enforce: 'pre', resolveId(id) { return id.startsWith('@tauri-apps/') ? fixture : undefined; } }],
});
let child;
let socket;
let sequence = 0;
...
try {
  await server.listen();
  const base = server.resolvedUrls.local[0] + 'scripts/fixtures/windows-compat.html';
  ...
  async function capture(name) {
    ...
    const state = await evaluate(`({ denied: window.__compat.denied, text: document.body.innerText,
      overflow: ['html','.modal-content','.modal-body','.oauth-content','.oauth-status','.oauth-link-panel','.tray-popup'].filter(selector=>{
    ...
    if (korean) assert.ok(!/[\u3400-\u9fff]/.test(state.text), `${name}: untranslated UI: ${state.text}`);
  ...
  await navigate('?view=proxy');
  await waitFor('!!document.querySelector(".copy-command-button")');
  assert.ok((await evaluate('document.querySelector(".copy-command-button code").textContent')).startsWith('$env:OPENAI_BASE_URL='));
  await evaluate('window.__compat.copyFailure=true'); await click('.copy-command-button'); await delay(150);
  assert.equal(await evaluate('window.__compat.clipboard'), null);
  assert.equal(await evaluate('!!document.querySelector(".copy-command-button .lucide-check")'), false, 'Failed copy reported success');
  await capture('proxy-copy-failure');
  await evaluate('window.__compat.copyFailure=false'); await click('.copy-command-button');
  await waitFor('window.__compat.clipboard !== null');
  await capture('proxy-powershell-copy');
  await navigate('?view=accounts');
  await waitFor('!!document.querySelector(".toolbar-icon-btn")');
  assert.equal(await evaluate('document.querySelector(".toolbar-icon-btn").disabled'), true, 'macOS-only action exposed on Windows');
  assert.equal(await evaluate('getComputedStyle(document.querySelector(".drag-handle")).cursor'), 'default');
  await capture('accounts-platform-guards');
  for (const variant of ['', '&unknown', '&long']) {
    await metrics(380, 410, 1.5);
    await navigate('?view=tray' + variant);
    await waitFor('!!document.querySelector(".tp-account")');
    if (variant === '&unknown') {
      assert.equal(await evaluate('document.querySelectorAll(".tp-status.unknown").length'), 2);
      assert.equal(await evaluate('document.querySelectorAll(".tp-status.critical").length'), 0);
    }
    await evaluate('document.querySelector(".tp-actions").scrollIntoView({block:"end"})');
    assert.equal(await evaluate('document.querySelector(".tp-actions").getBoundingClientRect().bottom <= innerHeight+1'), true);
    await capture('tray-380x410' + (variant || '-data').replace('&', '-'));
  }
  await evaluate('window.__compat.backendFailure=true; window.__compat.emit("accounts-updated")');
  await waitFor('!!document.querySelector(".tp-error")');
  await capture('tray-backend-error');
  assert.deepEqual(errors, [], 'Unhandled browser exception');
  console.log(`Windows UI regression: ${scenes.length} scenes passed; native IPC and account data mocked.`);
} finally {
